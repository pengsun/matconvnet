function mnist_small_tr()
%% init dag: from file or from scratch
beg_epoch = 4;
dir_mo = 'C:\Dev\code\matconvnet\examples_dag\mo_zoo\mnist_small';
fn_mo = fullfile(dir_mo, sprintf('dag_epoch_%d.mat', beg_epoch-1) );
if ( exist(fn_mo, 'file') )
  h = create_dag_from_file (fn_mo, beg_epoch-1);
else
  h = create_dag_from_scratch ();
end
%% config 
% TODO: add more properties here
h.beg_epoch = beg_epoch;
h.num_epoch = 5;
h.batch_sz = 128;
h.dir_mo = 'C:\Dev\code\matconvnet\examples_dag\mo_zoo\mnist_small';
fn_data = 'C:\Dev\code\matconvnet\examples\data\mnist_small_cv5\imdb.mat';
%% do the training
[X, Y] = load_tr_data(fn_data);
train(h, X,Y);

function h = create_dag_from_scratch ()
h = convdag_lenet();
h = init_dag(h);
  
function ob = create_dag_from_file (fn_mo, epoch_cnt)
load(fn_mo, 'ob');

function [X,Y] = load_tr_data(fn_data)
load(fn_data);
ind_tr = find( images.set == 1 );

X = images.data(:,:,:, ind_tr);
Y = images.labels(:, ind_tr);