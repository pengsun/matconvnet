classdef tfw_gpu_lenetDropout < tfw_i
  ...
  methods 
    ...
    function ob = tfw_gpu_lenetDropout()
    % Initialize the DAG net
      ...
      tfs{8}        = tf_conv();
      tfs{8}.i      = tfs{7}.o;
      tfs{8}.p(1).a = gpuArray( f*randn(1,1,500,10, 'single') );
      tfs{8}.p(2).a = gpuArray( zeros(1,10,'single') );
      % 9: loss
      tfs{9}      = tf_loss_lse();
      tfs{9}.i(1) = tfs{8}.o;
    ...
    function ob = fprop(ob)
       ...
       ob.tfs{1}.i.a    = gpuArray( ob.i(1).a ); %
       ob.tfs{9}.i(2).a = gpuArray( ob.i(2).a ); %
